...
fn create_rock() -> Actor {
    ...
    Actor {
        tag: ActorType::Rock,
        pos: Point2::ZERO,
        facing: (rng.rand_range(0..628) as f32) / 100.0,
        velocity: Vector2::ZERO,
        ang_vel: 0.,
        bbox_size: ROCK_BBOX,
        life: ROCK_LIFE,
...
fn player_brake(actor: &mut Actor) {
    actor.velocity -= actor.velocity * 0.03;
}
...
impl Assets {
    fn new(ctx: &mut Context) -> GameResult<Assets> {
        ...
        let player_thrust = graphics::Image::from_path(ctx, "/player_thrusting.png")?;
    ...
    fn actor_image(&self, actor: &Actor) -> &graphics::Image {
        match actor.tag {
            ActorType::Player => if actor.velocity.length() < 10.0 { &self.player_image } else { &self.player_thrust },
